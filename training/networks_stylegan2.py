...
def dense_layer(x, fmaps, gain=1, use_wscale=True, lrmul=1, weight_var='weight'):
    ...
    w = get_weight([tf.compat.dimension_value(x.shape[1]), fmaps], gain=gain, use_wscale=use_wscale, lrmul=lrmul, weight_var=weight_var)
    w = tf.cast(w, x.dtype)
...
def conv2d_layer(x, fmaps, kernel, up=False, down=False, resample_kernel=None, gain=1, use_wscale=True, lrmul=1, weight_var='weight'):
    ...
    w = get_weight([kernel, kernel, tf.compat.dimension_value(x.shape[1]), fmaps], gain=gain, use_wscale=use_wscale, lrmul=lrmul, weight_var=weight_var)
    if up:
        ...
        x = tf.nn.conv2d(input=x, filters=tf.cast(w, x.dtype), data_format='NCHW', strides=[1,1,1,1], padding='SAME')
    return x
...
def apply_bias_act(x, act='linear', alpha=None, gain=None, lrmul=1, bias_var='bias'):
    b = tf.compat.v1.get_variable(bias_var, shape=[tf.compat.dimension_value(x.shape[1])], initializer=tf.compat.v1.initializers.zeros()) * lrmul
...
def naive_downsample_2d(x, factor=2):
    with tf.compat.v1.variable_scope('NaiveDownsample'):
        ...
        return tf.reduce_mean(input_tensor=x, axis=[3,5])
...
def modulated_conv2d_layer(x, y, fmaps, kernel, up=False, down=False, demodulate=True, resample_kernel=None, gain=1, use_wscale=True, lrmul=1, fused_modconv=True, weight_var='weight', mod_weight_var='mod_weight', mod_bias_var='mod_bias'):
    ...
    # Modulate.
    s = dense_layer(y, fmaps=tf.compat.dimension_value(x.shape[1]), weight_var=mod_weight_var) # [BI] Transform incoming W to style.
...
def minibatch_stddev_layer(x, group_size=4, num_new_features=1):
    ...
    s = tf.compat.v1.shape(input=x)                                           # [NCHW]  Input shape.
    ...
    **_kwargs):                             # Ignore unrecognized keyword args.
    ...
    if normalize_latents:
        with tf.compat.v1.variable_scope('Normalize'):
            x *= tf.math.rsqrt(tf.reduce_mean(input_tensor=tf.square(x), axis=1, keepdims=True) + 1e-8)
    ...
    **_kwargs):                         # Ignore unrecognized keyword args.
    ...
    with tf.compat.v1.variable_scope('4x4'):
        with tf.compat.v1.variable_scope('Const'):
            x = tf.compat.v1.get_variable('const', shape=[1, nf(1), 4, 4], initializer=tf.compat.v1.initializers.random_normal())
    ...
    **_kwargs):                         # Ignore unrecognized keyword args.
    ...
    with tf.compat.v1.variable_scope('Output'):
        ...
        if labels_in.shape[1] > 0:
            x = tf.reduce_sum(input_tensor=x * labels_in, axis=1, keepdims=True)
    scores_out = x
