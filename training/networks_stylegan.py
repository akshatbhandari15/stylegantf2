...
def _upscale2d(x, factor=2, gain=1):
    #assert tf.rank(x) == 4 #and all(dim.value is not None for dim in x.shape[1:])
...
def dense(x, fmaps, **kwargs):
    if len(x.shape) > 2:
        x = tf.reshape(x, [-1, np.prod([tf.compat.dimension_value(d) for d in x.shape[1:]])])
    w = get_weight([tf.compat.dimension_value(x.shape[1]), fmaps], **kwargs)
    w = tf.cast(w, x.dtype)
...
def conv2d(x, fmaps, kernel, **kwargs):
    ...
    w = get_weight([kernel, kernel, tf.compat.dimension_value(x.shape[1]), fmaps], **kwargs)
    w = tf.cast(w, x.dtype)
...
def apply_noise(x, noise_var=None, randomize_noise=True):
    ...
    with tf.compat.v1.variable_scope('Noise'):
        ...
        weight = tf.compat.v1.get_variable('weight', shape=[x.shape.as_list()[1]], initializer=tf.compat.v1.initializers.zeros(), use_resource=False)
